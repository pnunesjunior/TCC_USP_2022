...
ggplot(data = dataset_diff, mapping = aes(x = log_itens)) +
  geom_histogram(binwidth = 1, color = "white", boundary = 0) +
  labs(title = "Difference in Differences",
       subtitle = "Histograma por total de itens ativos")+
  theme(plot.title = element_text(size = 20, color = "Black"),
        plot.subtitle = element_text(size = 10, color = "Black"))+
  xlab("Total itens ativos")+
  ylab("Total Sellers ")

# Total de itens ativos por grupo de controle e tratado
ggplot(data = dataset_diff, mapping = aes(x = log_itens)) +
  geom_histogram(binwidth = 1, color = "white", boundary = 0) +
  labs(title = "Difference in Differences",
       subtitle = "Histograma por total de itens ativos - Grupo Controle vs Tratado")+
  theme(plot.title = element_text(size = 20, color = "Black"),
        plot.subtitle = element_text(size = 10, color = "Black"))+
  xlab("Total itens ativos")+
  ylab("Total Sellers")+
...
ggplot(data = dataset_diff, mapping = aes(x = log_itens)) +
  geom_histogram(binwidth = 1, color = "white", boundary = 0) +
  labs(title = "Difference in Differences",
       subtitle = "Histograma por total de itens ativos - Por período")+
  theme(plot.title = element_text(size = 20, color = "Black"),
        plot.subtitle = element_text(size = 10, color = "Black"))+
  xlab("Total itens ativos")+
  ylab("Total Sellers")+
...
plot_data <- dataset_diff %>%
  ...
  summarize(mean_itens = mean(log_itens),
            se_itens = sd(log_itens) / sqrt(n()),
            upper = mean_itens + (1.96 * se_itens),
            lower = mean_itens + (-1.96 * se_itens))

ggplot(plot_data, aes(x = Seller_Emprestimo, y = mean_itens)) +
...
ggplot(plot_data, aes(x = Efeito_Tratamento, y = mean_itens, color = Seller_Emprestimo)) +
  geom_pointrange(aes(ymin = lower, ymax = upper), size = 1) +
  labs(title = "Difference in Differences",
       subtitle = "Indicador da média de itens ativos")+
  theme(plot.title = element_text(size = 20, color = "Black"),
        plot.subtitle = element_text(size = 10, color = "Black"))+
  xlab("Período das Vendas")+
  ylab("Média de itens ativos")+
...
########################################################################################
## Sobre as médias de itens ativos em cada período, antes do início da pandemia e depois do início da pandemia,
## e para cada um dos grupos, controle e tratado, observou-se que houve um crescimento mais acentuado na média de itens ativos
...
diffs <- dataset_diff %>%
  ...
  summarize(mean_itens = mean(log_itens),
            med_itens_ativos = mean(Total_Itens))
diffs

# Grupo de Tratamento - Pegou Emprestimo
# Grupo de Controle - Não pegou emprestimo

# Media de itens ativos de Sellers que pegaram emprestimo antes do inicio da pandemia - Grupo de Tratamento
antes_grupo_tratamento <- diffs %>%
  filter(Periodo_Venda == 0, Emprestimo == 1) %>%
  pull(mean_itens)

# Media de itens ativos  de Vendas de Sellers que não pegaram emprestimo antes do inicio da pandemia - Grupo de Controle
antes_grupo_controle <- diffs %>%
  filter(Periodo_Venda == 0, Emprestimo == 0) %>%
  pull(mean_itens)

# Media de itens ativos  de Sellers que pegaram emprestimo depois do inicio da pandemia - Grupo de Tratamento
depois_grupo_tratamento <- diffs %>%
  filter(Periodo_Venda == 1, Emprestimo == 1) %>%
  pull(mean_itens)

# Media de itens ativos de Sellers que não pegaram emprestimo depois do inicio da pandemia - Grupo de Controle
...
ggplot(diffs, aes(x = as.factor(Periodo_Venda), y = mean_itens,
                  color = as.factor(Emprestimo))) +
  geom_point() +
  labs(title = "Difference in Differences",
       subtitle = "Efeito da aquisição de emprestimo sobre ativação dos itens")+
  theme(plot.title = element_text(size = 20, color = "Black"),
        plot.subtitle = element_text(size = 10, color = "Black"))+
  xlab("Período das Vendas")+
  ylab("Média de Orders")+
...
## quantidade de itens ativos dos dados obtidos, por meio da metodologia Diff-in-Diff, observou-se que:

## 1. O efeito da aquisição do crédito provocou um aumento de 53% sobre a média de itens ativos do grupo tratado;
## 2. E que no geral, o grupo tratado ativou aproximadamente  28% a mais do que o grupo de controle após o efeito.
########################################################################################
...
modelo_simples <- lm(log_itens ~ Periodo_Venda + Emprestimo + (Periodo_Venda * Emprestimo),
                     data = dataset_diff)
tidy(modelo_simples)
...
modelos_variaveis <- lm(log_itens ~ Periodo_Venda + Emprestimo + (Periodo_Venda * Emprestimo) +
...
## não tiveram grande influência para efeito positivos na ativação de novos itens. O percentual da média de vendas do grupo
## tratado comparado ao grupo de controle ainda se mostra estável. Quando se considera todas as variáveis,
## há uma variação de 28% para 31%, ou seja, o crescimento das ativações não está correlacionado ao porte do
